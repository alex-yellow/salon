    ...
    var hsMain = {
        ...
        expand: function(el, params, custom, type) {
            if (type == 'html' && !el.hasAttribute('lg-uid')) {
                ...
                if (galleryContainer && !galleryContainer.hasAttribute('lg-uid')) {
                    ...
                    this.addLgEventListeners(el, params, custom, type);
                    el.click();
                } else if (!el.hasAttribute('lg-uid')) {
                    params = this.overrideParams(params);
        ...
        lgGalleryPreInit: function(el, params, custom) {
            ...
            for (var link of galleryContainer.querySelectorAll(this.lgImageSelector)) {
                if ( link.nextElementSibling && link.nextElementSibling.matches('.highslide-caption') ) {
                    link.dataset.subHtml = link.nextElementSibling.innerHTML;
                };
            };
        },
        ...
        lgHtmlInit: function(el, params, custom) {
            lightGallery(el, {
                ...
                hash: false,
                share: false,
                width: (params && params.width) ? params.width : '420px',
                height: (params && params.height) ? params.height : 'auto',
                mode: 'lg-fade',
                addClass: (params && params.objectType == 'iframe' && (!params.height || params.height == 'auto')) ? 'lg-iframe-auto' : 
                    (params && params.objectType == 'iframe') ? 'lg-iframe' : 'lg-html',
                counter: false,
                download: false,
                zoom: false,
                fullScreen: false,
